...
const { VoiceStatus } = require('discord.js').Constants;
...
class StreamDispatcher extends VolumeInterface {
  constructor(player, stream, streamOptions) {
     ...
     *
     */
    this.packetProcessor = (packet) => {
      if (packet.bytes > this.packetSize) this.packetSize = packet.bytes;
    ...
    this.stream = stream;
    this.packets = [];
    this.packetSize = 160; // 64kbps 20ms frame
    this.stream.on('data', this.packetProcessor);
    this.startStreaming();
  ...
  readStreamBuffer() {
    ...
    //if (packet.bytes !== this.packetSize) {
    //  const newPacket = Buffer.alloc(this.packetSize).fill(0);
    //  packet.packet.copy(newPacket);
    //  return newPacket;
    //}
  ...
  destroy(type, reason) {
    ...
    this.destroyed = true;
    this.setSpeaking(false);
    if (this.stream) this.stream.removeListener('data', this.packetProcessor);
